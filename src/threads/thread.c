...
/* Return thread with tid if it exist else return null */
struct thread *
...
/*Return child with tid if it exist in the list else return null */
struct child *
...
/* Return file elem with fd if it exist else return null */
...
/* Waketime comparator for two threads */
bool
waketime_compare (struct list_elem *elem1, struct list_elem* elem2, void *aux)
{ 
  struct thread* thread1 = list_entry(elem1, struct thread, elem); /* Thread 1 */
  struct thread* thread2 = list_entry(elem2, struct thread, elem); /* Thread 2 */
  /* Return true if wake_time of thread1 is less than or equal to thread 2, else retuen false */
...
thread_sleep (int64_t ticks)
{
  ...
  ASSERT(cur->status == THREAD_RUNNING); /* Current thread should be RUNNING thread */

  cur->wake_time = timer_ticks() + ticks; /* Assigning wake_up time */
  old_level = intr_disable(); /* Disable interrupt and stores its previous value */ 
  list_insert_ordered(&sleeping_list, &cur->elem, waketime_compare, NULL); /* Append the thread to sleeping_list in ascending order of wake_time */
  thread_block(); /* Blocks the current thread */
  intr_set_level(old_level); /* Set the interrupt to its previous value */
}

/* Checks the wakeup time of sleeping threads and wakes up the threads accordingly */
void
thread_wakeup ()
{ 
  /* Return if sleeping_list is empty */
  ...
  /* Iterate through the sleeping_list */
  while (elem1 != list_end(&sleeping_list)) {
    elem2 = list_next(elem1);
    t = list_entry(elem1, struct thread, elem); /* Thread corresponding to elem1 */

    if (timer_ticks() >= t->wake_time) { /* Check if the thread should be woken up */
      old_level = intr_disable(); /* Disable the interrupt and store its previous value */
      list_remove(elem1); /* Remove the thread from sleeping_list */
      thread_unblock(t); /* Unblock the thread */
      intr_set_level(old_level); /* SEt interrupt to its previous value */
      elem1 = elem2; /* Go to next element of the sleeping_list */
    } else {
      break; /* Break if the current iteration thread should not be woken up as it is in sorted order */ 
    }
  }
}
